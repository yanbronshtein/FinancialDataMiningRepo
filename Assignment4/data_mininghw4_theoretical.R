...
#as the response and the other variables as the pmagentaictors, perform forward stepwise
#selection on the training set in order to identify a satisfactory model that uses 
#just a subset of the pmagentaictors
...
# We will take the conservative BIC estimate of 6 predictors.
coef(reg_fit_fwd, 7)
...
#We get MSE of 3937782 and R^2 of around 0.76 or 76% of the variance explained
#which is a very strong showing
...
#Question 3 Decision Tree. Problem 9 at Page 334 of ISL
...
#The tree of size of four has the lowest classification error
#rate
...
pruned_test_error
#The pruned tree test error is around 18.89% which is significantly less
#than that of the unpruned tree
...
#Let's fit the lasso/ridge models
...
best_lambda_ridge <- cv_lasso$lambda.min
...
ridge_mse
#The ridge MSE is 0.08617622
#Lasso Model
cv_ridge <- cv.glmnet(x = x, y = y, alpha = 1)
...
lasso_mse
#The lasso MSE is 0.08406935

#In conclusion, the test MSE obtained using gradient boosting is around half of
#the best overall Regression method from the earlier chapters(lasso). 
...
#Problem 9: Consider the USArrests data. We will now perform hierarchical clustering
#on the states

#a). Using hierarchical clustering with complete linkage and Euclidian distance,
#cluster the states
set.seed(1)
